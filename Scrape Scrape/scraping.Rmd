...
my_url_arms <- "https://en.wikipedia.org/wiki/Arms_industry"
...
```{r}
# World Pop 
my_url <- "https://www.worldometers.info/world-population/population-by-country/"

temp <- my_data <- read_html(my_url) %>% 
	html_nodes("table") %>%
 			html_table()

my_data <- do.call(cbind,unlist(temp, recursive = FALSE)) %>% 
	as_tibble() %>% 
	select(-"#")
	
colnames(my_data)[1] <- "Country"

#Remove , from 1000 index
my_data <- lapply(my_data, function(x){gsub(",","",x)})
...
```{r}
#Remove Spaces
colnames(my_data) <- gsub(" ","", x  = colnames(my_data))
#remove everything within paranthasese 
colnames(my_data) <- gsub(r"{\s*\([^\)]+\)}","",colnames(my_data))
#Remove %
colnames(my_data) <- gsub("%","",colnames(my_data))

#Turn numeric everything but first col
my_data <- my_data %>% 
	mutate_at(vars(-colnames(my_data)[1]), as.numeric)
```

%% Visual investigation
I have a hypothesis that the countries with the largest populations tends to get more than 2 babies per. fertile woman compared to lower population countries. 

```{r}
my_data %>% 
	mutate(Fert_above_2 = if_else(Fert.Rate > 2, 1, 0)) %>% 
	ggplot(aes( Fert_above_2, fill = as.factor(Fert_above_2))) + geom_bar()


my_data %>% 
	mutate(Fert_above_2 = if_else(Fert.Rate > 2, 1, 0)) %>% 
	ggplot(aes(y = Fert_above_2, x = Population, col = as.factor(Fert_above_2))) + geom_point()+ geom_jitter() + labs(title = "Fert Above 2 ~ Population")
```
There are no obvious relationship between population size and if a fertility per fertile woman is above 2. 